  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 sm:px-6">
      ...
      {career.scholarships.length > 0 && (
        <section className="mt-8">
          <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900">
            <BadgeIndianRupee className="h-5 w-5 text-rose-600" />
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {career.scholarships.map((s) => (
              <Link
                key={s}
                href="/scholarships"
                className="rounded-full border border-gray-200 bg-white px-4 py-1.5 text-sm text-gray-700 shadow-sm transition-colors hover:border-rose-300 hover:bg-accent-light"
              >
                {s}
              </Link>
            ))}
