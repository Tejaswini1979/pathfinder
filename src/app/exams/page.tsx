...
import { getExams, type ExamPattern } from "@/data/exams";
...
function PatternDetails({ pattern }: { pattern: ExamPattern }) {
  const facts = [
    { label: "Marks", value: pattern.marks },
    { label: "Duration", value: pattern.duration },
    { label: "Mode", value: pattern.mode },
  ].filter((f) => f.value);
    ...
    <dl className="mt-2.5 space-y-3 text-sm">
      {pattern.structure && (
        <div>
          ...
          <dd className="mt-0.5 font-medium text-gray-700">{pattern.structure}</dd>
        </div>
      )}
        ...
        <div>
          ...
          <dd className="mt-1.5 flex flex-wrap gap-1.5">
            {pattern.sections.map((s) => (
              <span
                key={s}
                className="rounded-md border border-gray-200 bg-white px-2 py-0.5 text-xs font-medium text-gray-700"
              >
                {s}
              </span>
            ))}
        ...
        <div>
          ...
          <dd className="mt-1.5 space-y-1">
            {pattern.stages.map((s, i) => (
              <div key={s} className="flex items-start gap-1.5 text-xs font-medium text-gray-700">
                <span className="mt-0.5 font-semibold text-gray-400">{i + 1}.</span>
                {s}
              </div>
            ))}
          </dd>
      ...
      {facts.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-0.5">
          {facts.map((f) => (
            <span
              key={f.label}
              className="rounded-md bg-primary-light px-2 py-0.5 text-xs font-semibold text-primary"
            >
              {f.label}: {f.value}
            </span>
          ))}
      ...
      {pattern.notes && (
        <p className="text-xs leading-5 text-gray-500">{pattern.notes}</p>
      )}
    </dl>
  );
}
